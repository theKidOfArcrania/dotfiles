...
IGNORES=()
if [ -f ~/.global_ignores ]; then
  readarray IGNORES < ~/.global_ignores
fi
git_ps1() {
  for ignore in "${IGNORES[@]}"; do
    ignore_full=$(builtin cd /; realpath "$ignore")
    if [ "${PWD##$ignore_full}" != "$PWD" ]; then
      PS1="$1$2"
      return
    fi
  done
...
#fi
export PROMPT_COMMAND='git_ps1 "\[\e[0;32m\]\u@\h \[\e[0;33m\]\w " " \[\e[1;34m\]\n\$\[\e[0m\] ";'$PROMPT_COMMAND
